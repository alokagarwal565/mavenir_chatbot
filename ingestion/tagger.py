...
def generate_chunk_tags(
    ...
    """
    Generates deterministic 7-layer hierarchical taxonomy tags for a 3GPP chunk:
    - Layer 1: Domain / Subsystem
    - Layer 2: Procedure / Functional Topic
    - Layer 3: Network Node / Function
    - Layer 4: Interface / Reference Point
    - Layer 5: Protocol / Serialization
    - Layer 6: 3GPP Release
    - Layer 7: Normative Requirement Level
    """
    tags: Set[str] = set()

    # Layer 1: Domain / Subsystem
    if "23.501" in spec_number:
        tags.add("domain:architecture")
        tags.add("domain:core_5gc")
    elif "23.502" in spec_number:
        tags.add("domain:procedures")
        tags.add("domain:core_5gc")
    elif "24.501" in spec_number:
        tags.add("domain:signaling")
        tags.add("domain:core_5gc")
    elif "38.331" in spec_number or "38.300" in spec_number or "38.401" in spec_number:
        tags.add("domain:radio_nr")
    elif "33.501" in spec_number or "33.535" in spec_number:
        tags.add("domain:security")
    elif "29.5" in spec_number:
        tags.add("domain:sbi_apis")
        tags.add("domain:core_5gc")

    combined_text = f"{section_title or ''} {text[:500]}"
    ...
    # Layer 5: Protocols & Serialization
    for proto_tag, pat in PROTO_VOCAB.items():
        if pat.search(combined_text):
            tags.add(proto_tag)

    # Layer 6: 3GPP Release (if provided or detected)
    if release:
        tags.add(f"rel:{release}")

    # Layer 7: Normative Requirement Level & Information Type
    if "|" in text and "-|-" in text:
        tags.add("type:parameter_table")
    if re.search(r'\b(SHALL|SHALL NOT|MUST)\b', text, re.IGNORECASE):
        tags.add("type:normative_rule")
        tags.add("normative:mandatory")
    elif re.search(r'\b(SHOULD|SHOULD NOT|RECOMMENDED)\b', text, re.IGNORECASE):
        tags.add("normative:recommended")
    elif re.search(r'\b(MAY|OPTIONAL)\b', text, re.IGNORECASE):
        tags.add("normative:optional")

    if re.search(r'^\s*\d+\.\s+', text, re.MULTILINE):
        tags.add("type:call_flow_steps")
