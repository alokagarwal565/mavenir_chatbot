...
```sql
-- Core Schema Architecture (PostgreSQL 16 + pgvector)
...
CREATE TABLE specifications (
    ...
    series_number VARCHAR(5) NOT NULL,
    spec_type VARCHAR(5) NOT NULL,
    title TEXT NOT NULL,
    primary_wg VARCHAR(20),
...
CREATE TABLE canonical_documents (
    ...
    ast_storage_path TEXT,
    ingested_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE TABLE document_chunks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    document_id UUID REFERENCES canonical_documents(id) ON DELETE CASCADE,
